...
def edit_product(id):
    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            file.save(file_path)
            conn.execute("UPDATE products SET name=?, category=?, description=?, price=?, image=? WHERE id=?",
                         (name, category, description, price, file_path, id))
        else:
            conn.execute("UPDATE products SET name=?, category=?, description=?, price=? WHERE id=?",
